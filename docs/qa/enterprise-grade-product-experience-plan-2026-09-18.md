...
| 2026-09-18 | Phase 1 | Setup Templates production-readiness slice completed: API now returns template item contents, Platform Admin shows setup item counts/content status, and the adoption form previews item types, ordered items, dependencies, and safe adoption behavior before Apply. Frontend remains backward-compatible with currently deployed APIs that do not yet return `items`. | `backend/tests/test_tenant_onboarding_api.py`: `15 passed`; `platform-admin-tabs-pagination-certification.spec.ts` + `platform-admin-audit-evidence-certification.spec.ts`: `2 passed (1.0m)`; `pnpm --dir web exec tsc --noEmit`; `pnpm --dir web lint` |
| 2026-09-18 | Phase 1 | Setup Template item authoring slice completed: Platform Admin can add cloneable template items from the browser for leave types, leave policies, shifts, holiday calendars, and attendance policies with JSON payload validation and dependency keys. Backend exposes `POST /platform-policy-packs/{pack}/items/` and returns updated pack contents. | `backend/tests/test_tenant_onboarding_api.py`: `16 passed`; `platform-admin-tabs-pagination-certification.spec.ts` + `platform-admin-audit-evidence-certification.spec.ts`: `2 passed (1.0m)`; `pnpm --dir web exec tsc --noEmit`; `pnpm --dir web lint` |
| 2026-09-18 | Phase 1 | Setup Template guardrail slice completed: publishing zero-item templates now requires explicit header-only confirmation, and item authoring validates duplicate keys, dependency keys, and required payload fields per item type before records can be saved. | `backend/tests/test_tenant_onboarding_api.py`: `18 passed`; `platform-admin-tabs-pagination-certification.spec.ts` + `platform-admin-audit-evidence-certification.spec.ts`: `2 passed (1.0m)`; `pnpm --dir web exec tsc --noEmit`; `pnpm --dir web lint` |
