...
| 2026-09-17 | RBAC-6F Employee-backed Payroll/Statutory Personas | Complete locally | Staging failure showed HR Admin payroll/statutory setup endpoints require an active employee context. Updated the browser proof to create a real employee and provision access through HR Admin API setup before logging in as each limited RBAC persona; removed the setup-only Employees page visit that could land on the public workspace chooser. Local web typecheck passed and local no-env Playwright skip remains clean. |
