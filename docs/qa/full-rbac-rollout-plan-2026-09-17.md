...
| Platform permission catalog | Implemented locally | Catalog definitions can now be synced into `iam.PermissionCatalogEntry`; Platform Admin catalog UI reads DB rows when present and falls back to the deployed code catalog until sync is complete. |
...
Status: Safe staging sweep complete; destructive lockout proof deferred to disposable tenant  
...
| 2026-09-17 | RBAC-1B Platform Admin Permission Catalog Deployed Proof | Complete on deployed staging | `PLAYWRIGHT_BASE_URL=https://hrms.accerio.in PLAYWRIGHT_LIVE_SEED_PASSWORD=Password@123 npx playwright test tests/e2e/platform-admin-permission-catalog-certification.spec.ts --project=chromium --workers=1` passed `1/1`, proving deployed catalog visibility, search, assignability filter, risk filter, and platform-only permission visibility. |
...
| 2026-09-17 | RBAC-9 Safe Staging Sweep | Complete on deployed staging | `web/scripts/run-rbac9-staging.sh` completed successfully on deployed staging. Coverage: Tenant Admin `3 passed, 1 skipped`; HR leave/attendance `2 passed`; MSS custom approvers `2 passed`; payroll/statutory `3 passed`; payroll lifecycle `3 passed`. The single skipped case is the intentionally destructive last-admin lockout browser proof, reserved for a disposable tenant. |
...
| 2026-09-17 | RBAC-10A DB-Backed Permission Catalog Bridge | Complete locally | Added `PermissionCatalogEntry` model/migration/admin, `sync_permission_catalog` management command, DB-prefer/code-fallback Platform Admin catalog API behavior, catalog source tagging, and Platform Admin source visibility. Runtime permission enforcement intentionally remains code-backed until the governed edit/approval workflow exists. Focused backend catalog tests, Django check, migration check, and web typecheck are green. |
| 2026-09-17 | RBAC-10B Platform Catalog Metadata Editing | Complete locally | Added platform-staff PATCH API for catalog metadata, DB row bootstrap from the code catalog, clear validation for label/module/risk, source tagging for platform updates, and an inline Platform Admin edit dialog. Permission keys remain immutable and runtime enforcement remains code-backed. Focused backend catalog tests and web typecheck are green. |
| 2026-09-17 | RBAC-10C DB-Backed Workspace Menus | Complete locally | Added `MenuCatalogEntry` model/migration/admin, `sync_menu_catalog` command, authenticated menu catalog API, DB-first frontend menu loader with permission filtering and static fallback, and wired Platform Admin, Tenant Admin, HR Admin, ESS, MSS, and Finance Manager shell menus to DB rows. Focused backend menu proof, Django check, migration check, and web typecheck are green. |

## Current Known Gaps

| Gap | Risk | Target Phase |
| --- | --- | --- |
| Platform Admin permission catalog has DB-backed metadata editing, but no approval/version workflow yet | Platform team can maintain labels, module, risk, active state, assignability, and plan/module metadata; adding/removing runtime permission keys still needs governed versioning before launch use | RBAC-10D |
| Workspace menus are DB-backed but not yet editable from a Platform Admin menu-management screen | Menus can be synced and administered through Django admin/DB rows; launch UX still needs a governed Platform Admin screen for menu row edits, ordering, and permission review | RBAC-10D |
