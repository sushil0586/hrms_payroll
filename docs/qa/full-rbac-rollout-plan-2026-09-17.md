...
| 2026-09-17 | RBAC-6F Employee-backed Payroll/Statutory Personas | Complete locally | Staging failure showed HR Admin payroll/statutory setup endpoints require an active employee context, and the seeded admin lacked `employees.create`. Updated the browser proof to create a temporary setup role with `employees.create` and `employees.access.manage`, attach it to the seed admin membership, create a real employee, and provision access before logging in as each limited RBAC persona. Local web typecheck passed and local no-env Playwright skip remains clean. |
