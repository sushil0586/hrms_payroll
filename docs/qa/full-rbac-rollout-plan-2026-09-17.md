...
| Frontend permission context is implemented for Tenant Admin, MSS approvals/control center, core HR employee/org/document/leave/attendance pages, and payroll/statutory output/handoff/statutory setup action slices | Remaining downstream payroll workspaces still need a final lifecycle button sweep; payroll/statutory viewer proof is green against staging API with patched local UI | RBAC-6F/RBAC-7 |
