...
| PLF-6 Provider certification | In progress | PLF-6A locally certified with typecheck, production build, and browser suite `2 passed` covering provider center navigation, page actions, live-rail disabled messaging, metrics, connection selection, certification/rehearsal controls, storage/package/client/adapter/mapping/simulation/certification registers, provider evidence export, evidence manifest, no secret leakage, responsive overflow, and employee denial. | Pending deploy and live certification | 82% |
