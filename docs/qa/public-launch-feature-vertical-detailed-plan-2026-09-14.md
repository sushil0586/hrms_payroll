...
| PLF-6 Provider certification | In progress | PLF-6A deployed at `48fbc82`; post-deploy smoke passed, and live browser suite `2 passed` covering provider center navigation, page actions, live-rail disabled messaging, metrics, connection selection, certification/rehearsal controls, storage/package/client/adapter/mapping/simulation/certification registers, provider evidence export, evidence manifest, no secret leakage, responsive overflow, and employee denial. First live attempt saw a transient redirect to `/login` while selecting a provider connection; clean rerun passed. PLF-6B locally certified with typecheck, production build, and browser suite `2 passed` covering failure taxonomy buckets, callback/retry/job evidence, selected-provider event ledger, guarded retry/requeue/revoke API behavior, sensitive-field evidence checks, responsive overflow, and employee denial. | Remaining PLF-6 work: provider category setup expansion, live-enable approval gate, and deeper provider failure remediation actions against real failed delivery fixtures. | 89% |
