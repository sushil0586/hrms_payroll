...
export async function PATCH(request: NextRequest, { params }: Props) {
  ...
  const upstream = await fetch(`${API_BASE_URL}/hr-admin/notification-events/${itemId}/`, { method: "PATCH", headers: { "Content-Type": "application/json", Authorization: `Token ${token}` }, body: JSON.stringify(body), cache: "no-store" });
