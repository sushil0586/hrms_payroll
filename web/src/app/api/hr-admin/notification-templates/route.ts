...
export async function POST(request: NextRequest) {
  ...
  const upstream = await fetch(`${API_BASE_URL}/hr-admin/notification-templates/`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Token ${token}` }, body: JSON.stringify(body), cache: "no-store" });
