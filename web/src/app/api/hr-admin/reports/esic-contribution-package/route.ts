...
function textIncludesEsi(value: unknown) {
  return typeof value === "string" && /esi|esic|insurance|insured|employee_state_insurance/i.test(value);
}

function isEsiLine(line: Record<string, unknown>) {
  return textIncludesEsi([line.statutory_type, line.statutory_component_code, line.component_code, line.component_name, line.statutory_treatment_ref].join(" "));
}
...
function registrationForLine(line: Record<string, unknown>, registrations: Array<Record<string, unknown>>) {
  ...
  return registrations.find((item) => textIncludesEsi([item.registration_type_ref, item.registration_number, item.provider_ref, item.filing_authority_ref].join(" "))) ?? null;
}
...
function filingForLine(line: Record<string, unknown>, filings: Array<Record<string, unknown>>) {
  ...
  return filings.find((item) => textIncludesEsi([item.code, item.name, item.filing_type_ref, item.output_profile_ref, item.provider_ref].join(" "))) ?? null;
}
...
function numberFrom(line: Record<string, unknown>, keys: string[]) {
...
export async function GET(request: NextRequest) {
    ...
    .flatMap((artifact) =>
      (artifact.line_snapshot ?? [])
        ...
        .map((line) => {
          ...
          const employeeContribution = numberFrom(line, ["employee_contribution", "employee_esi", "amount"]);
          const employerContribution = numberFrom(line, ["employer_contribution", "employer_esi"]);
          return {
            record_type: "esic_contribution_member",
            contribution_ref: "india.esic.contribution.configurable",
            filing_code: stringValue(line.filing_code || filing?.code) || "summary",
            filing_type_ref: stringValue(line.filing_type_ref || filing?.filing_type_ref) || "esic.contribution",
            employer_esi_number: stringValue(line.employer_registration_number || line.registration_number || registration?.registration_number),
            ...
            esi_number: stringValue(line.esi_number || profile?.esi_number),
            ...
            esi_wages: String(numberFrom(line, ["esi_wages", "wage_base", "gross_wages"]) || ""),
            employee_contribution: String(employeeContribution || ""),
            employer_contribution: String(employerContribution || ""),
            total_contribution: String(employeeContribution + employerContribution || ""),
            statutory_type: stringValue(line.statutory_type) || "employee_state_insurance",
  ...
  const esiApplicableProfiles = profiles.filter((profile) => booleanValue(profile.esi_applicable));
  const blockingReasons = [];
  if (!rows.length) blockingReasons.push("No published ESIC statutory report rows are available.");
  if (esiApplicableProfiles.length > 0 && esiApplicableProfiles.some((profile) => !stringValue(profile.esi_number))) blockingReasons.push("ESIC number is missing for one or more ESI-applicable employee profiles.");
  if (rows.some((row) => !row.employer_esi_number)) blockingReasons.push("Employer ESIC registration is missing for one or more rows.");
  ...
  if (blockingReasons.length) {
    return NextResponse.json({ detail: "ESIC contribution package is not ready.", blocking_reasons: blockingReasons }, { status: 400 });
  }
  ...
  if (request.nextUrl.searchParams.get("format") === "manifest") {
    return NextResponse.json(
      {
        package_schema_version: "hrms.esic.contribution.package.manifest.v1",
        report_key: "esic-contribution-package",
      ...
      },
      {
        headers: {
          ...
          "X-HRMS-Report-Key": "esic-contribution-package",
  ...
  return new NextResponse(csv, {
    ...
    headers: {
      ...
      "Content-Disposition": 'attachment; filename="esic-contribution-package.csv"',
      "X-HRMS-Package-Checksum": packageChecksum,
      "X-HRMS-Report-Key": "esic-contribution-package",
      "X-HRMS-Source-Row-Count": String(rows.length),
    },
