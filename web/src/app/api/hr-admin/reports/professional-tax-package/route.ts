...
function isPtLine(line: Record<string, unknown>) {
  return textIncludesPt([line.statutory_type, line.statutory_component_code, line.component_code, line.component_name, line.statutory_treatment_ref].join(" "));
}
...
function registrationForLine(line: Record<string, unknown>, registrations: Array<Record<string, unknown>>) {
  ...
  return registrations.find((item) => textIncludesPt([item.registration_type_ref, item.registration_number, item.provider_ref, item.filing_authority_ref].join(" "))) ?? null;
}
...
function filingForLine(line: Record<string, unknown>, filings: Array<Record<string, unknown>>) {
  ...
  return filings.find((item) => textIncludesPt([item.code, item.name, item.filing_type_ref, item.output_profile_ref, item.provider_ref].join(" "))) ?? null;
}
...
export async function GET(request: NextRequest) {
    ...
    .flatMap((artifact) =>
      (artifact.line_snapshot ?? [])
        ...
        .map((line) => {
          ...
          return {
            record_type: "professional_tax_return_employee",
            return_ref: "india.professional_tax.return.configurable",
            filing_code: stringValue(line.filing_code || filing?.code) || "summary",
            filing_type_ref: stringValue(line.filing_type_ref || filing?.filing_type_ref) || "professional_tax.return",
            pt_state: stringValue(line.professional_tax_state || line.pt_state || profile?.professional_tax_state),
            employer_pt_number: stringValue(line.employer_registration_number || line.registration_number || registration?.registration_number),
            ...
            gross_wages: amountFor(line, ["gross_wages", "wage_base", "pt_wages"]),
            professional_tax_amount: amountFor(line, ["professional_tax_amount", "pt_amount", "amount"]),
            statutory_type: stringValue(line.statutory_type) || "professional_tax",
  ...
  if (!rows.length) blockingReasons.push("No published Professional Tax statutory report rows are available.");
  if (profiles.length > 0 && profiles.some((profile) => !stringValue(profile.professional_tax_state))) blockingReasons.push("Professional Tax state is missing for one or more employee profiles.");
  if (rows.some((row) => !row.pt_state)) blockingReasons.push("Professional Tax state is missing for one or more rows.");
  if (rows.some((row) => !row.employer_pt_number)) blockingReasons.push("Employer Professional Tax registration is missing for one or more rows.");
  ...
  if (blockingReasons.length) return NextResponse.json({ detail: "Professional Tax package is not ready.", blocking_reasons: blockingReasons }, { status: 400 });
  ...
  if (request.nextUrl.searchParams.get("format") === "manifest") {
    return NextResponse.json(
      {
        package_schema_version: "hrms.professional_tax.package.manifest.v1",
        report_key: "professional-tax-package",
      ...
      { headers: { "X-HRMS-Package-Checksum": packageChecksum, "X-HRMS-Report-Key": "professional-tax-package", "X-HRMS-Source-Row-Count": String(rows.length) } },
    );
  }
  ...
  return new NextResponse(csv, {
    ...
    headers: {
      ...
      "Content-Disposition": 'attachment; filename="professional-tax-package.csv"',
      "X-HRMS-Package-Checksum": packageChecksum,
      "X-HRMS-Report-Key": "professional-tax-package",
