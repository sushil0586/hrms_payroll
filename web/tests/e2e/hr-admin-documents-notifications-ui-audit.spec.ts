...
function normalizeDocumentsNotificationsHref(rawHref: string, baseUrl: string) {
  ...
  if (!documentsNotificationsPathPatterns.some((pattern) => pattern.test(url.pathname))) {
    return null;
  }
...
async function expectVisibleDocumentsNotificationsLinksHealthy(page: Page, route: string) {
  ...
  const normalizedHrefs = [
    ...new Set(hrefs.map((href) => normalizeDocumentsNotificationsHref(href, page.url())).filter((href): href is string => Boolean(href))),
  ];

  for (const href of normalizedHrefs) {
    const response = await page.request.get(href, { failOnStatusCode: false, maxRedirects: 2 });
    expect(response.status(), `${route} exposes a document/notification/import link that does not open cleanly: ${href}`).toBeLessThan(400);
  }
}

async function expectVisibleDocumentDownloadsHealthy(page: Page, route: string) {
  const downloadHrefs = await page.locator("main a[href^='/api/hr-admin/']").evaluateAll((links) =>
    links
      .filter((link) => {
  ...
  for (const href of [...new Set(downloadHrefs)]) {
    const response = await page.request.get(href, { failOnStatusCode: false, maxRedirects: 2 });
    expect(response.status(), `${route} exposes a document download that does not open cleanly: ${href}`).toBeLessThan(400);
  }
}
...
async function auditDocumentsNotificationsRoute(page: Page, route: string) {
  ...
  await expectVisibleDocumentsNotificationsLinksHealthy(page, route);
