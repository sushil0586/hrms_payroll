...
const reportsOperationsPathPatterns = reportsOperationsRoutes.map((route) => new RegExp(`^${route.replaceAll("/", "\\/")}$`));

function normalizeReportsOperationsHref(rawHref: string, baseUrl: string) {
  const url = new URL(rawHref, baseUrl);
  if (url.origin !== new URL(baseUrl).origin) {
    return null;
  }
  if (!reportsOperationsPathPatterns.some((pattern) => pattern.test(url.pathname))) {
    return null;
  }
...
async function expectVisibleReportsOperationsLinksHealthy(page: Page, route: string) {
  ...
  const normalizedHrefs = [
    ...new Set(hrefs.map((href) => normalizeReportsOperationsHref(href, page.url())).filter((href): href is string => Boolean(href))),
  ];

  for (const href of normalizedHrefs) {
    const response = await page.request.get(href, { failOnStatusCode: false, maxRedirects: 2 });
    expect(response.status(), `${route} exposes an operations/report link that does not open cleanly: ${href}`).toBeLessThan(400);
  }
}

async function expectVisibleReportExportsHealthy(page: Page, route: string) {
  const exportHrefs = await page.locator("main a[href^='/api/hr-admin/reports/']").evaluateAll((links) =>
    links
      .filter((link) => {
        const rect = link.getBoundingClientRect();
        const style = window.getComputedStyle(link);
  ...
  for (const href of [...new Set(exportHrefs)]) {
    const response = await page.request.get(href, { failOnStatusCode: false, maxRedirects: 2 });
    expect(response.status(), `${route} exposes a report export that does not open cleanly: ${href}`).toBeLessThan(400);
  }
}
...
async function expectNoVisibleControlCollisions(page: Page, route: string) {
  ...
  expect(collisions, `${route} has clipped or overlapping reports/operations controls`).toEqual([]);
}

async function auditReportsOperationsRoute(page: Page, route: string) {
  ...
  await expectVisibleReportsOperationsLinksHealthy(page, route);
  await expectVisibleReportExportsHealthy(page, route);
}

test.describe("HR Admin reports, audit, and operations UI audit", () => {
  test("certifies reports/operations page layout, controls, links, and exports", async ({ page }) => {
    test.setTimeout(480_000);
    await page.setViewportSize({ width: 1440, height: 960 });

    for (const route of reportsOperationsRoutes) {
      await auditReportsOperationsRoute(page, route);
    }
  });
});
