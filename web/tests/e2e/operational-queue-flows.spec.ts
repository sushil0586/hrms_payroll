...
test.describe("HR admin operational queue flows", () => {
  ...
  test("lifecycle inbox filters records and blocks demo bulk owner updates", async ({ page }) => {
    await gotoAuthenticated(page, "/hr-admin/lifecycle");
    ...
    const openRecord = page.getByRole("link", { name: "Open record" }).first();
    if (await openRecord.isVisible().catch(() => false)) {
      await expect(openRecord).toHaveAttribute("href", /onboardings/);
    }
    ...
    if (await selectPage.isEnabled().catch(() => false)) {
      ...
      await expect(page.getByText("Bulk owner updates are disabled in demo mode.").or(page.getByText("Owner updated.")).first()).toBeVisible();
    }
    await expectNoHorizontalOverflow(page);
  });

  test("employee document queue filters expiry work and selects reminder candidates", async ({ page }) => {
    await gotoAuthenticated(page, "/hr-admin/employee-documents");
    ...
    if (await selectPage.isEnabled().catch(() => false)) {
      ...
      await expect(page.getByText("No documents found").or(page.getByText("Select page")).first()).toBeVisible();
    }
    await expectNoHorizontalOverflow(page);
  });

  test("notification queue filters pending notifications and selects retry candidates", async ({ page }) => {
    await gotoAuthenticated(page, "/hr-admin/notifications");
    ...
    await expect(page.getByRole("heading", { name: "New leave request pending approval" }).first()).toBeVisible();
