...
test.describe("Phase R5-A payroll input exceptions report certification", () => {
  test("HR admin can certify payroll input exception filters, pagination, export evidence, and drilldowns", async ({ page }) => {
    ...
    for (const column of ["Employee", "Run", "Readiness", "Inputs", "Lock", "Evidence", "Actions"]) {
    ...
    for (const label of ["Payroll run", "Snapshot status", "Pay group", "Lock state", "Issue type"]) {
    ...
    for (const sort of ["risk", "employee", "run", "status", "locked"]) {
    ...
    await expect(filteredExportLink).toHaveAttribute("href", /\/api\/hr-admin\/reports\/payroll-input-exceptions\?.*sort=risk/);
    ...
    expect(filteredExportResponse.headers()["x-hrms-report-key"]).toBe("payroll-input-exceptions");
    ...
    expect(manifestResponse.headers()["x-hrms-report-key"]).toBe("payroll-input-exceptions");
  ...
  test("employee cannot access payroll input exceptions report or exports", async ({ page }) => {
    ...
    const csvResponse = await page.request.get("/api/hr-admin/reports/payroll-input-exceptions?sort=risk");
